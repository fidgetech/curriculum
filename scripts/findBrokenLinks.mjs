...
import { sendMail } from './resend.mjs';
        ...
        for (let url of allUrls) {
          ...
          try {
            ...
            if (error?.response?.status !== 403) {
              ...
              console.log(`${abridgedPath} - ${abridgedUrl} - ${error}`);
...
async function distributeLogFile() {
  if (process.env.NODE_ENV === 'production') {
    await sendMail({
      subject: 'Fidge tech offline links/images',
      html: '<p>See  attached file for a best guess list of broken links and images.</p>',
      filePath: logFile
    });
  } else {

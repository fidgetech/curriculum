---
title: "📓 3.4.0.15 Structuring Data in Firestore"
day: weekend
id: 3-4-0-15-structuring-data-in-firestore
...
## Writing Down the Structure in `types.ts`
---

Firestore won't enforce a schema for us, but our own code can hold us to one. Whichever of the three structures we pick, the plan should end up written out as types in `src/types.ts`. For the multiple-collections approach, our trails app might look like this:
...
export type NewTrail = Omit<Trail, "id">;
export type NewReview = Omit<Review, "id">;
```

This is the same pattern we set up for tickets: one type per collection describing a saved record, and an `Omit<..., "id">` version describing a record on its way into the database, where Firestore is about to assign the identifier.

Notice that the `trailId` field on `Review` is the association itself. In a SQL database, a foreign key constraint would refuse to store a review pointing at a trail that doesn't exist. TypeScript can't do that, because it only checks our code and never looks at the database. What it _can_ do is guarantee that every review we build has a `trailId`, that the value is a string, and that we never misspell the field name. That's a meaningful share of the mistakes this structure invites, caught before the app ever runs.

The nested-document structure from earlier in this lesson can be described with types too. A nested review has no `trailId`, because the trail it belongs to is the document containing it:

```ts title="src/types.ts"
export type NestedReview = {
  content: string;
};
...
Writing these types out is a useful design exercise on its own. If a type is awkward to write down, that's often a sign the data structure behind it is awkward too.

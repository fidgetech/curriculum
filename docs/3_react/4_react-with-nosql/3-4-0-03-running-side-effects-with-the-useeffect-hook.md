---
title: "📓 3.4.0.3 Running Side Effects with the useEffect Hook"
day: weekend
id: 3-4-0-3-running-side-effects-with-the-useeffect-hook
hide_table_of_contents: true
---

We already know how to use `useState` to manage state inside a function component. Now let's learn about the companion hook for running **side effects**: `useEffect`.
...
As always with a new Vite project, open `tsconfig.app.json` and add `"strict": true` to `compilerOptions` if it isn't there already. With the dev server running, replace the code in `src/App.tsx` with:
...
And here's `src/Counter.tsx`, the same as in the Introduction to State lesson:
...
Note that we don't need to import React itself. Vite's React template uses the automatic JSX runtime, which means JSX works without an `import React from 'react'` line. We only import the specific hooks we use, like `useState`.
...
Neither `useState` call needs a type argument here. TypeScript infers `number` from the initial value of `0` and `boolean` from the initial value of `false`.
...
We import `useEffect` alongside `useState` at the top of the file, then call it inside the component. It takes a callback function as its argument, and that callback is the "effect" that runs. If we run our app and click the _Count!_ or _Hide/Show_ buttons, we'll see `"effect!"` logged each time, because every state change causes a re-render, and every re-render triggers the effect.
...
With `document.title = String(count)`, we're updating the `<title>` tag of our HTML to the current value of `count`. Now if we click the _Count!_ or _Hide/Show_ buttons, we'll see `"effect!"` logged each time and the page title will update to match `count`.

:::note
Notice that we wrap `count` in `String()` rather than assigning it directly. `document.title` is typed as a `string`, and `count` is a `number`, so `document.title = count` is a type error:

```
Type 'number' is not assignable to type 'string'.
```

This is a small example of TypeScript catching a mismatch that plain JavaScript would have quietly converted for us. `String(count)` makes the conversion explicit, which is exactly what we want.
:::
...
We can tell the `useEffect` hook to run its effect only once by passing in an empty dependency array:
...
Note that we now return two elements from `App`, so we wrap them in a fragment (`<>...</>`). A component can only return a single root element, and the fragment groups our two components together without adding an extra element to the DOM. When there's only one element to return, like the single `<Counter />` we had before, no fragment is needed.

Next, let's create a new file called `Timer.tsx`, also in the `src` folder. Here's the code that we'll add inside of `Timer.tsx` to create the new `Timer` component:
...
Notice the annotation on our `interval` variable: `let interval: number | undefined;`. In the browser, `setInterval()` returns a number that identifies the interval, and that's the value we hand to `clearInterval()` later. We have to annotate it here because we declare `interval` before we know whether we'll assign anything to it. If `isActive` is `false`, we never call `setInterval()` at all, so `interval` stays `undefined`. Writing `number | undefined` describes both possibilities, and `clearInterval()` accepts either one.
...
When we pass a function to a state setter, React passes the current state value in as the argument (here we name it `timerState`). This lets us update `timer` without listing it as a dependency in the effect, which means the effect won't re-run every second just because `timer` changed.

We don't have to annotate `timerState` either. Because `timer` was initialized with `0`, TypeScript knows this state holds a `number`, so it infers that `timerState` is a `number` and that our function has to return one. Try changing the body to `timerState + "1"` and watch TypeScript object.

For more detail on this pattern, see [Updating state based on the previous state](https://react.dev/reference/react/useState#updating-state-based-on-the-previous-state) in the React docs.
:::

## Resources and Next Steps
---

Whew! We've covered a lot in this lesson. If anything about `useEffect` isn't feeling entirely clear, know that we'll be using it again when we connect Help Queue to Firebase, and seeing it applied to a real problem will help it click. Before we start in on that, we're going to wrap up our introduction to hooks by reviewing the rules of hooks and how to write a custom hook.

Note that it's normal for unexpected things to happen when we're first learning about hooks and how to implement them. If you run into issues, you should always start by referencing [the React docs on Hooks](https://react.dev/reference/react/hooks).
